...
def test_render_outputs_are_strict_and_create_safe_parents(tmp_path: Path) -> None:
    ...
    outputs = canonical_render_outputs(
        {
            "fullSize": str(target / f"{ASSET_ID}_fullsize_fuji_{REVISION}_edited.jpeg"),
            "preview": str(target / f"{ASSET_ID}_preview_fuji_{REVISION}_edited.jpeg"),
            "thumbnail": str(target / f"{ASSET_ID}_thumbnail_fuji_{REVISION}_edited.webp"),
        },
...
def test_render_output_rejects_wrong_kind_and_format(tmp_path: Path) -> None:
    ...
    with pytest.raises(PathPolicyError, match="invalid edited-derivative"):
        canonical_render_outputs(
            {
                "fullSize": str(root / f"{ASSET_ID}_preview_fuji_{REVISION}_edited.jpeg"),
                "preview": str(root / f"{ASSET_ID}_preview_fuji_{REVISION}_edited.jpeg"),
                "thumbnail": str(root / f"{ASSET_ID}_thumbnail_fuji_{REVISION}_edited.webp"),
            },
            {"fullSize": "jpeg", "preview": "jpeg", "thumbnail": "webp"},
            REVISION,
            root,
...
def test_cleanup_is_bounded_to_regular_edited_derivatives(tmp_path: Path) -> None:
    root = tmp_path / "thumbs"
    root.mkdir()
    fuji_target = root / f"{ASSET_ID}_preview_fuji_{REVISION}_edited.jpeg"
    ...
    symlink = root / f"{ASSET_ID}_thumbnail_fuji_{REVISION}_edited.webp"

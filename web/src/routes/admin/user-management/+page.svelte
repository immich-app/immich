...
<table class="text-left w-full my-5">
	...
	>
		{#each data.allUsers as user (user.id)}
			<tr
				class={`text-center flex place-items-center w-full h-[80px] dark:text-immich-dark-fg even:bg-immich-gray even:dark:bg-immich-dark-gray/75 odd:bg-immich-bg odd:dark:bg-immich-dark-gray/50 ${
					isDeleted(user) ? 'bg-red-300 dark:bg-red-900' : ''
				}`}
			>

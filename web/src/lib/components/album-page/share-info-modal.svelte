<script lang="ts">
  ...
  const handleRemoveUser = async () => {
    ...
    try {
      ...
      const message = userId === 'me' ? `Left ${album.albumName}` : `Removed ${selectedRemoveUser.fullName}`;
    ...
    <section class="immich-scrollbar max-h-[400px] overflow-y-auto pb-4">
      <div class="flex w-full place-items-center justify-between gap-4 p-5">
        <div class="flex place-items-center gap-4">
          ...
          <p class="text-sm font-medium">{album.owner.fullName}</p>
        </div>
...
{#if selectedRemoveUser && selectedRemoveUser?.id !== currentUser?.id}
  <ConfirmDialogue
    title="Remove User?"
    prompt="Are you sure you want to remove {selectedRemoveUser.fullName}"
    confirmText="Remove"
    on:confirm={handleRemoveUser}
    on:cancel={() => (selectedRemoveUser = null)}
  />

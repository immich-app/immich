<script lang="ts">
  ...
  import { mdiClose, mdiLicense } from '@mdi/js';
...
{#if getAccountAge() > 14}
  <div class="hidden md:block license-status pl-4 text-sm">
    {#if !$isPurchased}
      ...
      >
        <div
          class="flex gap-1 mt-2 place-items-center dark:bg-immich-dark-primary/10 bg-gray-100 py-3 px-2 border supporter-border rounded-lg"
        >
          <div class="h-5 w-5">
            <ImmichLogo noText />
          </div>
...
<Portal target="body">
  {#if showMessage}
    <div
      class="w-72 absolute bottom-[75px] left-[255px] bg-white dark:bg-gray-800 dark:text-white text-black rounded-xl z-10 shadow-2xl px-4 py-5"
    ...
    >
      ...
      <div class="mt-3 flex flex-col gap-1">
        ...
        <Button size="sm" fullwidth on:click={openLicenseModal} color="primary-inversed"
          >{$t('license_button_buy_license')}</Button
        >
      </div>
    </div>
  {/if}

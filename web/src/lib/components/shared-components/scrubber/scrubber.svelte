    ...
    >
      {#if $isTimelineScrolling && formatedDate}
        <p
          ...
          class="truncate opacity-1 pointer-events-none absolute right-0 bottom-0 z-[100] min-w-20 max-w-64 w-fit rounded-tl-md border-b-2 border-immich-primary bg-immich-bg/90 py-1 px-1 text-sm font-medium shadow-[0_0_8px_rgba(0,0,0,0.25)] dark:border-immich-dark-primary dark:bg-immich-dark-gray/90 dark:text-immich-dark-fg"
        >
          {formatedDate}
        </p>
      {/if}

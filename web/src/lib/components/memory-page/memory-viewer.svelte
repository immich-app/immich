<script lang="ts">
  ...
  const parseIndex = (s: string | null, max: number | null) => Math.max(Math.min(Number.parseInt(s ?? '') || 0, max ?? 0), 0);
        ...
        <div class="flex place-content-center place-items-center gap-2 overflow-hidden">
          ...
          {#each currentMemory.assets as _, index}
            <button
              ...
              on:click={() => goto(`?${QueryParameter.MEMORY_INDEX}=${memoryIndex}&${QueryParameter.ASSET_INDEX}=${index}`)}
            >

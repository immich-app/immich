        ...
        <form class="flex w-full max-h-56 gap-1" on:submit|preventDefault={() => handleSendComment()}>
          <div class="flex w-full items-center gap-4">
            <textarea
              ...
              bind:this={textArea}
              bind:value={message}
              placeholder={canComment ? 'Say something' : 'Comments are disabled'}
              on:input={autoGrow}
              on:keypress={handleEnter}
              class="h-[18px] {canComment
                ? ''
                : 'cursor-not-allowed'} w-full max-h-56 pr-2 items-center overflow-y-auto leading-4 outline-none resize-none bg-gray-200"
            />
          </div>
          {#if isSendingMessage}

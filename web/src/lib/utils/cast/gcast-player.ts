...
class GCastPlayer {
  static instance: GCastPlayer | null = null;
  ...
  static getInstance(): GCastPlayer {
    if (this.instance) {
      return this.instance;
    }

    this.instance = new GCastPlayer();
  ...
  async loadMedia(mediaUrl: string) {
    ...
    const successCallback = this.onMediaDiscovered.bind(GCastPlayer.instance, SESSION_DISCOVERY_CAUSE.LOAD_MEDIA);
    ...
    try {
      const data = await createApiKey({
        apiKeyCreateDto: {
          ...
          // TODO: Make this a finer grained permission
  ...
  private async getCastApiKey() {
    ...
    const previousKey = currentKeys.find((key) => key.name == CAST_API_KEY_NAME);

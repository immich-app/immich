...
async function addSupportedMimeTypes(): Promise<void> {
  ...
  } else if (globalThis.isSecureContext && typeof ImageDecoder !== 'undefined') {
    const isJxlSupported = await ImageDecoder.isTypeSupported('image/jxl');
    if (isJxlSupported) {
      supportedImageMimeTypes.add('image/jxl');
    }

    const isHeicSupported = await ImageDecoder.isTypeSupported('image/heic');
    if (isHeicSupported) {

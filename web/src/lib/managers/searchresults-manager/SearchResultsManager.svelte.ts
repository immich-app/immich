...
export class SearchResultsManager extends PhotostreamManager {
  ...
  async init() {
    ...
    await this.initTask.execute(() => {
      ...
      for (let i = 1; i < 3; i++) {
        this.#months.push(new SearchResultsSegment(this, { ...this.#options.searchTerms, page: i }));
      }
  ...
  loadNextPage() {
    ...
    this.#months.push(new SearchResultsSegment(this, { ...this.#options.searchTerms, page: this.#months.length + 1 }));
    this.updateViewportGeometry(false);
  }

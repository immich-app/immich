...
<Modal icon={mdiTune} size="giant" title={$t('search_options')} {onClose}>
  <ModalBody>
    <form id={formId} autocomplete="off" {onsubmit} {onreset}>
      <div class="flex flex-col gap-3" tabindex="-1">
        <SearchFilterGroup title={$t('content')}>
          <SearchFilterSection title={$t('search_type')} icon={mdiMagnify} expanded>
        ...
        <SearchFilterGroup title={$t('metadata')}>
        ...
        <SearchFilterGroup title={$t('options')}>

...
export function focusTrap(container: HTMLElement, options?: Options) {
  ...
  const setInitialFocus = async () => {
    ...
    const focusableElement = allTabbable.find((el) => !el.hasAttribute('data-focus-trap'));
  ...
  const getFocusableElements = () => {
    ...
    const focusableElements = allTabbable.filter((el) => !el.hasAttribute('data-focus-trap'));

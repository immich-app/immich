...
2. Set `IMMICH_CAST_RECEIVER_APP_ID` to the resulting application ID in the Immich server environment, then restart the server. Both apps read the ID from the server. Without a configured ID, they use Google's Default Media Receiver, subject to the web build setting described below.

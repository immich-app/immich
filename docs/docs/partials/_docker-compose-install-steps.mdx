...
Download [`docker-compose.yml`](https://github.com/open-noodle/gallery/releases/latest/download/docker-compose.yml) and [`example.env`](https://github.com/open-noodle/gallery/releases/latest/download/example.env) by running the following commands:
...
From the directory you created in Step 1 (which should now contain your customized `docker-compose.yml` and `.env` files), run the following command to start Gallery as a background service:

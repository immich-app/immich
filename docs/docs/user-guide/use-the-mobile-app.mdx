...
Installing the app, signing in, and the settings that control what it uploads. For how mobile backup behaves — deduplication, networking, album sync, and platform differences — see [Mobile Backup](/concepts/mobile-backup).
...
Provided the server is healthy and [backed up](/administration/back-up-and-restore), assets removed by Free Up Space can always be accessed in the Immich app.
...
For the semantics of the sync — what matches, what merges, and what is not propagated — see [Mobile Backup](/concepts/mobile-backup#album-synchronization-semantics).

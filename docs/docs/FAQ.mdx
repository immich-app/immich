...
### Why are features not working with a self-signed certificate, Basic Auth, or mutual TLS?

These network features are experimental. They often do not work with video playback, asset upload, and other features.
...
Instead of these experimental features, we recommend using a VPN or a [free trusted SSL certificate](https://letsencrypt.org/) for your domain.

...
### Which EXIF Tags does Immich support?

- Make, Model, Exifimagewidth, Exifimageheight, Filesizeinbyte, Orientation, Datetimeoriginal, Modifydate, Len, Fnumber, Focallength, Iso, Latitude, Longitude, City, State, Country, Description, Fps, Exposuretime, Livephotocid, Timezone, Projectiontype, Profiledescription, Colorspace, Bitspersample.

### Does Immich change the file?

No, Immich does not touch the original file under any circumstances,  
all changes of the file are saved in the Side Car file and in the database.
...
### Files from WhatsApp do not appear with a correct date, why?

Files sent on WhatsApp are saved without metadata on the file, 
therefore Immich has no way of knowing what the original date of the file is when files are uploaded from WhatsApp not in the order of arrival on the device.

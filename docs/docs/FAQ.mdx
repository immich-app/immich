...
However, Immich will delete trashed original files when the trash is emptied in the Immich UI.
...
In external libraries mounted as read-only (`:ro`), Immich is unable to create the `.xmp` sidecar files to store edited file metadata.
...
Immich will attempt to delete original files when the trash is emptied. In read-write external libraries (the default), Immich will delete the original file.
In read-only libraries, files can be trashed in the web UI, but when the trash is emptied, they will re-appear in the main timeline since Immich is unable to delete the original file.

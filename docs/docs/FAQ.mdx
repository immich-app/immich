...
### How can I set a user quota below 1 GB?

You can set the quota to a number of bytes using the API.
Here, an example of 1 byte is used.

```bash title="Set user quota"
# Get list of user IDs
curl https://immich.domain.tld/api/admin/users -H 'Accept: application/json' -H 'x-api-key: YOURAPIKEYHERE'

# Set quota to 1 byte for specified ID
curl -X PUT https://immich.domain.tld/api/admin/users/USERIDHERE \
-H 'Content-Type: application/json' -H 'Accept: application/json' -H 'x-api-key: YOURAPIKEYHERE' \
--data-raw '{ "quotaSizeInBytes": 1 }'
```

...
No, it currently does not hide or filter NSFW images from the library. This fork can optionally detect NSFW images and add searchable metadata, but timeline, search, and album filtering are separate future privacy features. There is an [open feature request on Github](https://github.com/immich-app/immich/discussions/2451).

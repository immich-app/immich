...
The Postgres database files (`DB_DATA_LOCATION`) must be located on a filesystem that supports user/group
ownership and permissions. This means that it will not work in a mounted directory (commonly under `/mnt`) in WSL (Windows Subsystem for Linux), or
on any filesystems formatted in NTFS or ex/FAT/32. If this is an issue, you can change the bind mount to a Docker volume instead.

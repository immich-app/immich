...
Immich only supports upgrading to the most recent stable release, and the server and mobile app have to be upgraded in a particular order — see [Versioning](/concepts/versioning) before a major-version upgrade.
...
Instances still using the deprecated pgvecto.rs database extension have to move to VectorChord. See [Migrate to VectorChord](/administration/migrate-to-vectorchord).

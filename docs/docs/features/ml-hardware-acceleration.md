...
- MIGraphX is a new backend for AMD cards. This new backend compiles models at runtime and offers caching to improve performance on subseuqnt inferences. This requires the following environment variables to be set:
  - `ORT_MIGRAPHX_CACHE_PATH` - caches model weights, is a file path (e.g. `/migraphx/weights`)
  - `ORT_MIGRAPHX_MODEL_CACHE_PATH` - caches the compilled models, is a file path (e.g. `/migraphx/models`)

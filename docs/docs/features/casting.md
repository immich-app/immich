...
With this receiver, the web app sends the current photo and its previous/next URLs to the Chromecast. The receiver keeps the current photo visible until the selected photo is ready. It downloads, decodes, and renders both adjacent photos into cached canvases, so selecting a prepared neighbor does not need another download or decode. The canvases are kept in receiver memory rather than displayed until selected. Looping videos use the receiver's video element instead of reloading the Cast queue on every repeat. This can avoid the queue's loading screen, although a device may still pause briefly when seeking back to the start of a video. Without `VITE_IMMICH_CAST_RECEIVER_APP_ID`, the web app continues to use Google's default receiver, which reloads videos on every loop. The mobile app currently continues to use the default receiver.

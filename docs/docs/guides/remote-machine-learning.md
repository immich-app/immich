...
:::info
If using hardware acceleration, the [hwaccel.ml.yml](https://github.com/open-noodle/gallery/releases/latest/download/hwaccel.ml.yml) file also needs to be added and the `docker-compose.yml` needs to be configured as described in the [hardware acceleration documentation](/features/ml-hardware-acceleration)
:::

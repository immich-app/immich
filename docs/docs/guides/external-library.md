...
This guide assumes you are running Gallery in Docker and that the files you wish to access are stored
...
If you want Gallery to be able to delete the images in the external library or add metadata ([XMP sidecars](/features/xmp-sidecars)), remove `:ro` from the end of the mount point.
...
Restart Gallery by running `docker compose up -d`.

...
describe(`/oauth`, () => {
  ...
  describe('POST /oauth/authorize', () => {
    ...
    it(`should throw an error if a redirect uri is not provided`, async () => {
      ...
      expect(body).toEqual(errorDto.validationError([{ path: ['redirectUri'], message: 'Invalid input: expected string, received undefined' }]));
    });
  ...
  describe('POST /oauth/callback', () => {
    ...
    it(`should throw an error if a url is not provided`, async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['url'], message: 'Invalid input: expected string, received undefined' }]));
    });

    it(`should throw an error if the url is empty`, async () => {
      const { status, body } = await request(app).post('/oauth/callback').send({ url: '' });
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['url'], message: 'Too small: expected string to have >=1 characters' }]));
    });
  ...
  describe(`POST /oauth/backchannel-logout`, () => {
    it(`should throw an error if the logout_token is not provided`, async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['logout_token'], message: 'Invalid input: expected string, received undefined' }]));
    });

...
describe('/map', () => {
  ...
  describe('GET /map/reverse-geocode', () => {
    ...
    it('should throw an error if a lat is not provided', async () => {
      ...
      expect(body).toEqual(errorDto.validationError([{ path: ['lat'], message: 'Invalid input: expected number, received NaN' }]));
    });
    ...
    it('should throw an error if a lat is out of range', async () => {
      const { status, body } = await request(app)
        ...
        .set('Authorization', `Bearer ${admin.accessToken}`);
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['lat'], message: 'Too big: expected number to be <=90' }]));
    });
    ...
    it('should throw an error if a lon is not provided', async () => {
      const { status, body } = await request(app)
        ...
        .set('Authorization', `Bearer ${admin.accessToken}`);
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['lon'], message: 'Invalid input: expected number, received NaN' }]));
    });

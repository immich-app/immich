...
describe('/libraries', () => {
  ...
  describe('POST /libraries', () => {
    ...
    it('should not create an external library with duplicate import paths', async () => {
      ...
      expect(body).toEqual(errorDto.validationError([{ path: ['importPaths'], message: 'Array must have unique items' }]));
    });
    ...
    it('should not create an external library with duplicate exclusion patterns', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['exclusionPatterns'], message: 'Array must have unique items' }]));
    });
  });
  ...
  describe('PUT /libraries/:id', () => {
    ...
    it('should not set an empty name', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['name'], message: 'Too small: expected string to have >=1 characters' }]));
    });
    ...
    it('should reject an empty import path', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['importPaths'], message: 'Array items must not be empty' }]));
    });
    ...
    it('should reject an empty exclusion pattern', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(errorDto.validationError([{ path: ['exclusionPatterns'], message: 'Array items must not be empty' }]));
    });
  });

  ...
  describe('POST /oauth/authorize', () => {
    ...
    it('should return a redirect uri', async () => {
      const { status, body } = await request(app)
        ...
        .send({ redirectUri: `http://127.0.0.1:${process.env.SERVER_PORT}/auth/login` });
      ...
      expect(params.get('redirect_uri')).toBe(`http://127.0.0.1:${process.env.SERVER_PORT}/auth/login`);
      expect(params.get('state')).toBeDefined();
    });
  });

...
describe('/duplicates', () => {
  ...
  describe('GET /duplicates', () => {
    it('should require authentication', async () => {
      const { status, body } = await request(app).get('/duplicates');
  ...
  describe('POST /duplicates/stack', () => {
    it('should require authentication', async () => {
      const { status, body } = await request(app)
        .post('/duplicates/stack')
        .send({
          duplicateId: uuidDto.dummy,
          assetIds: [uuidDto.dummy, uuidDto.dummy2],
        });
    ...
    it('should require valid duplicate group', async () => {
      const { status, body } = await request(app)
        .post('/duplicates/stack')
        .set('Authorization', `Bearer ${user1.accessToken}`)
        .send({
      ...
      expect(body).toEqual(errorDto.badRequest(expect.stringContaining('not found or access denied')));
    });

    it('should require at least two assets', async () => {
      const asset = await utils.createAsset(user1.accessToken);
      const duplicateId = '00000000-0000-4000-8000-000000000005';
      await utils.setAssetDuplicateId(user1.accessToken, asset.id, duplicateId);
    ...
    it('should create stack from duplicate group', async () => {
      const [asset1, asset2] = await Promise.all([
        utils.createAsset(user1.accessToken),
        utils.createAsset(user1.accessToken),
      ]);

      const duplicateId = '00000000-0000-4000-8000-000000000006';
      await utils.setAssetDuplicateId(user1.accessToken, asset1.id, duplicateId);
      await utils.setAssetDuplicateId(user1.accessToken, asset2.id, duplicateId);
      ...
      // Verify side effects: stack created and duplicateIds cleared
      const stackId = body.id;
      const [asset1Info, asset2Info] = await Promise.all([
        utils.getAssetInfo(user1.accessToken, asset1.id),
        utils.getAssetInfo(user1.accessToken, asset2.id),
      ]);

      expect(asset1Info.stack?.id).toBe(stackId);
      expect(asset1Info.duplicateId).toBeNull();
      expect(asset2Info.stack?.id).toBe(stackId);
      expect(asset2Info.duplicateId).toBeNull();
    });

    it('should respect primaryAssetId when creating stack', async () => {
      const [asset1, asset2] = await Promise.all([
        utils.createAsset(user1.accessToken),
        utils.createAsset(user1.accessToken),
      ]);

      const duplicateId = '00000000-0000-4000-8000-000000000007';
      await utils.setAssetDuplicateId(user1.accessToken, asset1.id, duplicateId);
      await utils.setAssetDuplicateId(user1.accessToken, asset2.id, duplicateId);
      ...
      expect(status).toBe(201);
      expect(body.primaryAssetId).toBe(asset2.id);
    });

    it('should reject assets not in duplicate group', async () => {
      const [asset1, asset2, asset3] = await Promise.all([
        utils.createAsset(user1.accessToken),
        utils.createAsset(user1.accessToken),
        utils.createAsset(user1.accessToken),
      ...
      const duplicateId = '00000000-0000-4000-8000-000000000008';
      await utils.setAssetDuplicateId(user1.accessToken, asset1.id, duplicateId);
      await utils.setAssetDuplicateId(user1.accessToken, asset2.id, duplicateId);
      // asset3 is NOT in the duplicate group
      ...
      const { status, body } = await request(app)
        ...
        .send({
          duplicateId,
          assetIds: [asset1.id, asset3.id],
        });

      expect(status).toBe(400);
      expect(body).toEqual(errorDto.badRequest(expect.stringContaining('not a member of duplicate group')));
    });

    it('should reject primaryAssetId not in assetIds', async () => {
      const [asset1, asset2, asset3] = await Promise.all([
        utils.createAsset(user1.accessToken),
        utils.createAsset(user1.accessToken),
        utils.createAsset(user1.accessToken),
      ...
      const duplicateId = '00000000-0000-4000-8000-000000000014';
      await utils.setAssetDuplicateId(user1.accessToken, asset1.id, duplicateId);
      await utils.setAssetDuplicateId(user1.accessToken, asset2.id, duplicateId);
      await utils.setAssetDuplicateId(user1.accessToken, asset3.id, duplicateId);
      ...
      const { status, body } = await request(app)
        ...
        .send({
          duplicateId,
          assetIds: [asset1.id, asset2.id],
          primaryAssetId: asset3.id, // asset3 is in group but not in assetIds
        });

      expect(status).toBe(400);
      expect(body).toEqual(errorDto.badRequest(expect.stringContaining('primaryAssetId must be in assetIds')));
    });

    it('should prevent access to other user duplicate groups', async () => {
      const [asset1, asset2] = await Promise.all([
        utils.createAsset(user2.accessToken),
        utils.createAsset(user2.accessToken),
      ]);

      const duplicateId = '00000000-0000-4000-8000-000000000009';
      await utils.setAssetDuplicateId(user2.accessToken, asset1.id, duplicateId);
      await utils.setAssetDuplicateId(user2.accessToken, asset2.id, duplicateId);

      // user1 tries to stack user2's duplicates
      const { status, body } = await request(app)
        .post('/duplicates/stack')
        .set('Authorization', `Bearer ${user1.accessToken}`)
        .send({

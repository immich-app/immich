...
describe('/server', () => {
  ...
  describe('GET /server/license', () => {
    ...
    it('should return the server license', async () => {
      ...
      await request(app).put('/server/license').set('Authorization', `Bearer ${admin.accessToken}`).send(license);
      const { status, body } = await request(app)
        .get('/server/license')
        .set('Authorization', `Bearer ${admin.accessToken}`);
      expect(status).toBe(200);
  ...
  describe('DELETE /server/license', () => {
    ...
    it('should return the server license', async () => {
      ...
      await request(app).delete('/server/license').set('Authorization', `Bearer ${admin.accessToken}`).send(license);

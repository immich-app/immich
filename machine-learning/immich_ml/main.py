...
from immich_ml.models.transforms import decode_pil, compute_phash
import math
...
@app.post("/phash", dependencies=[Depends(update_state)])
async def phash(image: bytes = File(...)) -> Any:
    """Return perceptual hash (pHash) for an image as 16-char lowercase hex."""
    pil = await run(lambda: decode_pil(image))
    hash_hex = await run(lambda: compute_phash(pil))
    return ORJSONResponse({"pHash": hash_hex})


@app.post("/auto-stack/score", dependencies=[Depends(update_state)])
async def auto_stack_score(payload: dict) -> Any:
    """Compute visual similarity aggregate for a set of embeddings and (optional) pHashes.

    Expected JSON body:
    {
      "embeddings": { "assetId": [..float..], ... },
      "pHashes": { "assetId": "16hex", ... }
    }
    Returns:
    { "avgCos": float, "pHashAvg": float|null, "blended": float } (visual scores normalized 0..1)
    """
    try:
        embeddings: dict[str, list[float]] = payload.get("embeddings", {})
        p_hashes: dict[str, str] = payload.get("pHashes", {})
        emb_items = list(embeddings.items())
        if len(emb_items) < 2:
            return ORJSONResponse({"avgCos": None, "pHashAvg": None, "blended": 0})
        # cosine pairwise
        ...
        for i in range(len(emb_items)):
            id_i, vec_i = emb_items[i]
            n_i = norm(vec_i)
            if not n_i:
                continue
            for j in range(i + 1, len(emb_items)):
                id_j, vec_j = emb_items[j]
                n_j = norm(vec_j)
                if not n_j:
                    continue
                ...
                cos_sum += dot / (n_i * n_j)
                pairs += 1
        avg_cos = cos_sum / pairs if pairs else 0.0
        # pHash similarity (average)
        ph_items = [h.lower() for h in p_hashes.values() if isinstance(h, str) and len(h) == 16]
        p_pairs = 0
        p_sum = 0.0
        if len(ph_items) > 1:
            def hamming(a: str, b: str) -> int:
                ...
                return bits
            total_bits = 64
            for i in range(len(ph_items)):
                for j in range(i + 1, len(ph_items)):
                    diff = hamming(ph_items[i], ph_items[j])
                    p_sum += 1 - diff / total_bits
                    p_pairs += 1
        p_hash_avg = p_sum / p_pairs if p_pairs else None
        clip_component = (avg_cos + 1) / 2.0
        if p_hash_avg is not None:
            blended = clip_component * 0.7 + p_hash_avg * 0.3
        else:
            blended = clip_component
        return ORJSONResponse({"avgCos": avg_cos, "pHashAvg": p_hash_avg, "blended": blended})
    except Exception as e:  # pragma: no cover - defensive
        raise HTTPException(400, f"Invalid payload: {e}")


@app.post("/auto-stack/score-batch", dependencies=[Depends(update_state)])
async def auto_stack_score_batch(payload: dict) -> Any:
    """Batch variant of visual similarity aggregate.

    Expected JSON body:
    { "groups": [ { "id": "g1", "embeddings": {..}, "pHashes": {..} }, ... ] }
    Returns: { "results": [ { id, avgCos, pHashAvg, blended }, ... ] }
    """
    try:
        ...
        for group in groups:
            gid = group.get("id")
            embeddings = group.get("embeddings", {}) or {}
            p_hashes = group.get("pHashes", {}) or {}
            emb_items = list(embeddings.items())
            if len(emb_items) < 2:
                results.append({"id": gid, "avgCos": None, "pHashAvg": None, "blended": 0})
                continue
            def norm(v: list[float]) -> float:
                return math.sqrt(sum(x * x for x in v)) or 0.0
            pairs = 0
            ...
            p_hash_avg = p_sum / p_pairs if p_pairs else None
            clip_component = (avg_cos + 1) / 2.0
            blended = clip_component * 0.7 + p_hash_avg * 0.3 if p_hash_avg is not None else clip_component
            results.append({"id": gid, "avgCos": avg_cos, "pHashAvg": p_hash_avg, "blended": blended})

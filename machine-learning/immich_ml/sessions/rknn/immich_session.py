...
try:
    from .native import rknn_pool as _native_mod
except ImportError:  # pragma: no cover - depends on local build
    _native_mod = None

_native: ModuleType | None = _native_mod
...
if TYPE_CHECKING:
    ...
    NativeRKNNExecutor = None if _native is None else _native.NativeRKNNExecutor
...
is_available = NativeRKNNExecutor is not None and soc_name is not None and settings.rknn
...
class RknnPoolExecutor:
    ...
    def submit(self, inputs: Sequence[Array32], *, tag: Any = None) -> Future[RKNNInferenceResult]:
        if self._closed:
            raise RuntimeError("Pool is closed")
        return self._executor.submit(self._run_inference, list(inputs), tag)

    def put(self, inputs: Sequence[Array32], *, tag: Any = None) -> Future[RKNNInferenceResult]:
...
class RknnSession:
    ...
    def _build_nodes(self, key: str) -> list[SessionNode]:
        ...
        for entry in self._io_info.get(key, []):
            ...
            nodes.append(RknnNode(name=entry.get("name"), shape=symbolic_shape))
        return nodes

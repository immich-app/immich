...
export interface IAssetRepository {
  ...
  getAssetsWithNoAutoGeneratedTags(): Promise<AssetEntity[]>;
  getExistingAssets(
    userId: string,
...
export class AssetRepository implements IAssetRepository {
  ...
  async getAssetsWithNoAutoGeneratedTags(): Promise<AssetEntity[]> {
    return await this.assetRepository
      .createQueryBuilder('asset')
      .leftJoinAndSelect('asset.tags', 'tag')
      .where(
        new Brackets((qb) => {
          qb.where('tag.id IS NULL').orWhere(`tag.type = 'CUSTOM'`);
        }),
      )

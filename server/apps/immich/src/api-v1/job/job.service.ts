...
export class JobService {
  ...
  private async runMachineLearningPipeline(): Promise<number> {
    ...
    const assetWithNoAutoGeneratedTags = await this._assetRepository.getAssetsWithNoAutoGeneratedTags();

    for (const asset of assetWithNoAutoGeneratedTags) {
    ...
    return assetWithNoAutoGeneratedTags.length;
  }
}

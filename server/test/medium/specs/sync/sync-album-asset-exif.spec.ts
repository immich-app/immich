...
describe.concurrent(SyncRequestType.PartnerAssetExifsV1, () => {
  ...
  it('should not sync album asset exif for own user', async () => {
    ...
    await expect(testSync(auth, [SyncRequestType.AlbumAssetExifsV1])).resolves.toHaveLength(0);
  });
  ...
  it('should backfill album assets exif when a user shares an album with you', async () => {
    ...
    const assetRepo = getRepository('asset');
    // Asset to check that we don't backfill our own assets
    ...
    await assetRepo.upsertExif({ assetId: asset1Owner.id, make: 'Canon' });
    await wait(2);
    await assetRepo.create(asset1User2);
    await assetRepo.upsertExif({ assetId: asset1User2.id, make: 'Canon' });
    await wait(2);
    await assetRepo.create(asset2User2);
    await assetRepo.upsertExif({ assetId: asset2User2.id, make: 'Canon' });
    await wait(2);
    await assetRepo.create(asset3User2);
    await assetRepo.upsertExif({ assetId: asset3User2.id, make: 'Canon' });
    ...
    await sut.setAcks(auth, { acks: [backfillResponse[2].ack, backfillResponse.at(-1).ack] });

...
describe(SearchService.name, () => {
  ...
  describe('aspect ratio filter', () => {
    it('should filter by minAspectRatioWidth/minAspectRatioHeight', async () => {
      const { sut, ctx } = setup();
      const { user } = await ctx.newUser();
      ...
      const auth = factory.auth({ user: { id: user.id } });
      const response = await sut.searchMetadata(auth, { minAspectRatioWidth: 2, minAspectRatioHeight: 1 });

      expect(response.assets.items).toEqual([expect.objectContaining({ id: wideAsset.id })]);
    });

    it('should filter by maxAspectRatioWidth/maxAspectRatioHeight', async () => {
      const { sut, ctx } = setup();
      const { user } = await ctx.newUser();
      ...
      const auth = factory.auth({ user: { id: user.id } });
      const response = await sut.searchMetadata(auth, { maxAspectRatioWidth: 1, maxAspectRatioHeight: 2 });

      expect(response.assets.items).toEqual([expect.objectContaining({ id: narrowAsset.id })]);
    });

    it('should filter by both min and max aspect ratio width/height', async () => {
      const { sut, ctx } = setup();
      const { user } = await ctx.newUser();
      ...
      const response = await sut.searchMetadata(auth, {
        minAspectRatioWidth: 1.2,
        minAspectRatioHeight: 1,
        maxAspectRatioWidth: 1.5,
        maxAspectRatioHeight: 1,
      });

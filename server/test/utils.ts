...
export const newTestService = <T extends BaseService>(
  ...
  // Helper to get mock with override support
  const getMock = <K extends keyof ServiceOverrides>(key: K): ServiceOverrides[K] => {
    return (overrides[key] || mocks[key]) as ServiceOverrides[K];
  };

  const sut = new Service(
    getMock('logger') as LoggingRepository,
    getMock('access') as IAccessRepository as AccessRepository,
    getMock('activity') as ActivityRepository,
    getMock('album') as AlbumRepository,
    getMock('albumUser') as AlbumUserRepository,
    getMock('apiKey') as ApiKeyRepository,
    getMock('app') as AppRepository,
    getMock('asset') as AssetRepository,
    getMock('assetEdit') as AssetEditRepository,
    getMock('assetJob') as AssetJobRepository,
    getMock('audit') as AuditRepository,
    getMock('config') as ConfigRepository,
    getMock('cron') as CronRepository,
    getMock('crypto') as CryptoRepository,
    getMock('database') as DatabaseRepository,
    getMock('downloadRepository') as DownloadRepository,
    getMock('duplicateRepository') as DuplicateRepository,
    getMock('email') as EmailRepository,
    getMock('event') as EventRepository,
    getMock('job') as JobRepository,
    getMock('library') as LibraryRepository,
    getMock('machineLearning') as MachineLearningRepository,
    getMock('map') as MapRepository,
    getMock('media') as MediaRepository,
    getMock('memory') as MemoryRepository,
    getMock('metadata') as MetadataRepository,
    getMock('move') as MoveRepository,
    getMock('notification') as NotificationRepository,
    getMock('oauth') as OAuthRepository,
    getMock('ocr') as OcrRepository,
    getMock('partner') as PartnerRepository,
    getMock('person') as PersonRepository,
    getMock('plugin') as PluginRepository,
    getMock('process') as ProcessRepository,
    getMock('search') as SearchRepository,
    getMock('serverInfo') as ServerInfoRepository,
    getMock('session') as SessionRepository,
    getMock('sharedLink') as SharedLinkRepository,
    getMock('sharedLinkAsset') as SharedLinkAssetRepository,
    getMock('stack') as StackRepository,
    getMock('storage') as StorageRepository,
    getMock('sync') as SyncRepository,
    getMock('syncCheckpoint') as SyncCheckpointRepository,
    getMock('systemMetadata') as SystemMetadataRepository,
    getMock('tag') as TagRepository,
    getMock('telemetry') as unknown as TelemetryRepository,
    getMock('trash') as TrashRepository,
    getMock('user') as UserRepository,
    getMock('versionHistory') as VersionHistoryRepository,
    getMock('view') as ViewRepository,
    getMock('websocket') as WebsocketRepository,
    getMock('workflow') as WorkflowRepository,
  );

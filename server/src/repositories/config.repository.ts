...
const asBoolean = (value: string | undefined, defaultValue: boolean) => (value ? value === 'true' : defaultValue);
...
const getEnv = (): EnvData => {
  ...
  const globalEnabled = asBoolean(process.env.IMMICH_METRICS, false);
  const hostMetrics = asBoolean(process.env.IMMICH_HOST_METRICS, globalEnabled);
  const apiMetrics = asBoolean(process.env.IMMICH_API_METRICS, globalEnabled);
  const repoMetrics = asBoolean(process.env.IMMICH_IO_METRICS, globalEnabled);
  const jobMetrics = asBoolean(process.env.IMMICH_JOB_METRICS, globalEnabled);

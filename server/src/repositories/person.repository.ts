...
type PersonGroupRow = {
  ...
  updatedAt: Date;
  updateId: string;
  ownedPerson: ShallowDehydrateObject<Selectable<PersonTable>> | null;
...
const asPerson = (userId: string, group: PersonGroupRow) => {
  const { personGroupId, createdAt, updatedAt, updateId, ownedPerson, otherPeople } = group;
  ...
  return {
    ...
    updatedAt: asDateTimeString(updatedAt),
    updateId,
    ...ownedPerson,
    otherPeople,
  };
...
export class PersonRepository {
  ...
  async getAllForUser(pagination: PaginationOptions, userId: string, options?: PersonSearchOptions) {
    // defaults for shared-only person records
    const isHidden = sql<boolean>`coalesce("owned"."isHidden", false)`;
    const isFavorite = sql<boolean>`coalesce("owned"."isFavorite", false)`;

    const items = await this.db
      .selectFrom('person_group')
      // the viewing user's own record for this person, which may not exist when the person is only shared with them
      ...
      // the person is either one of the viewing user's own, or one another user shared with them
      .where((eb) =>
        eb.or([
      ...
      .$if(!options?.closestFaceAssetId, (qb) =>
        qb
          ...
          .orderBy((eb) => eb.fn.coalesce('owned.createdAt', 'person_group.createdAt')),
      )
      .$if(!options?.withHidden, (qb) => qb.where('owned.isHidden', 'is not', true))
      .offset(pagination.skip ?? 0)
      .limit(pagination.take + 1)
      .execute();

    return paginationHelper(
      items.map((item) => asPerson(userId, item)),
      pagination.take,
    );
  }
  ...
  async getForUser({ userId, personGroupId }: { userId: string; personGroupId: string }) {
    ...
    return group && asPerson(userId, group);
  }

...
export class SyncRepository {
  ...
  getAlbumAssetDeletes(userId: string, ack?: SyncAck) {
    return this.db
      .selectFrom('album_assets_assets_audit')
  ...
  getAlbumAssetUpserts(userId: string, ack?: SyncAck) {
    return this.db
      ...
      .where('albums_assets_assets.updatedAt', '<', sql.raw<Date>("now() - interval '1 millisecond'"))
      .$if(!!ack, (qb) => qb.where('albums_assets_assets.updateId', '>', ack!.updateId))
      .orderBy('albums_assets_assets.updateId', 'asc')
      .where((eb) =>
        eb(

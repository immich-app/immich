...
import { Kysely, OrderByDirectionExpression, Selectable, sql } from 'kysely';
...
export class SearchRepository {
  ...
  async searchRandom(size: number, options: AssetSearchOptions) {
    ...
    const greaterThan = builder
      ...
      .orderBy(sql`random()`)
      .limit(size);
    const { rows } = await sql<Asset>`${lessThan} union all ${greaterThan} limit ${size}`.execute(this.db);
    return rows;
  }

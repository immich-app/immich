...
import { anyUuid, asUuid, upsertHelper } from 'src/utils/database';
...
export class AssetRepository implements IAssetRepository {
  ...
  async upsertExif(exif: Insertable<Exif>): Promise<void> {
    const values = { ...exif, assetId: asUuid(exif.assetId) };
    await upsertHelper(this.db, this.databaseRepository.getTable('exif'), values, ['assetId']).execute();
  }
  ...
  async upsertJobStatus(...jobStatus: Insertable<AssetJobStatus>[]): Promise<void> {
    ...
    await upsertHelper(this.db, this.databaseRepository.getTable('asset_job_status'), values, ['assetId']).execute();
  }
  ...
  async getByIds(
    ...
    const res = await this.db
      ...
      .$if(!!tags, (qb) => qb.select((eb) => withTags(eb)))
      .execute();
  ...
  async update(asset: Updateable<Assets> & { id: string }): Promise<AssetEntity> {
    const value = omitBy(asset, (v) => isUndefined(v));
    delete value.id;
    if (!isEmpty(value)) {
      return this.db
        ...
        .selectAll('assets')
        .$call((qb) => withExif(qb))
        .$call((qb) => qb.select((eb) => withFacesAndPeople(eb)))
  ...
  getStatistics(ownerId: string, { isArchived, isFavorite, isTrashed }: AssetStatsOptions): Promise<AssetStats> {
    return this.db
      ...
      .$if(!!isTrashed, (qb) => qb.where('assets.status', '=', AssetStatus.TRASHED))
    ...
    return ((options.personId ? hasPeopleCte(this.db, [options.personId]) : this.db) as Kysely<DB>)
      .with('assets', (qb) =>
        qb
          ...
          .$if(!!options.isTrashed, (qb) => qb.where('assets.status', '=', AssetStatus.TRASHED))
      ...
      this.db
        .with('duplicates', (qb) =>
          qb
            ...
            .select('duplicateId')
            .select((eb) =>
              eb.fn<Assets[]>('jsonb_strip_nulls', [eb.fn('jsonb_agg', [eb.table('assets')])]).as('assets'),
            )
            .where('ownerId', '=', asUuid(userId))
  ...
  async upsertFile(values: Pick<Insertable<AssetFiles>, 'assetId' | 'path' | 'type'>): Promise<void> {
    await upsertHelper(this.db, this.databaseRepository.getTable('asset_files'), values, ['assetId', 'type']).execute();
  }

  @GenerateSql({ params: [{ assetId: DummyValue.UUID, type: AssetFileType.PREVIEW, path: '/path/to/file' }] })
  async upsertFiles(values: Pick<Insertable<AssetFiles>, 'assetId' | 'path' | 'type'>[]): Promise<void> {
    await upsertHelper(this.db, this.databaseRepository.getTable('asset_files'), values, ['assetId', 'type']).execute();
  }
}

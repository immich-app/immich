...
import { anyUuid, asUuid, introspectUpsertColumns, upsertHelper } from 'src/utils/database';
...
export class AssetRepository implements IAssetRepository {
  constructor(
    ...
    introspectUpsertColumns(this.dataSource, 'exif', 'asset_job_status', 'asset_files');
  }

  async upsertExif(exif: Insertable<Exif>): Promise<void> {
    const values = [{ ...exif, assetId: asUuid(exif.assetId) }];
    await upsertHelper(this.db, 'exif', values, ['assetId']).execute();
  }
  ...
  async upsertJobStatus(...jobStatus: Insertable<AssetJobStatus>[]): Promise<void> {
    ...
    await upsertHelper(this.db, 'asset_job_status', values, ['assetId']).execute();
  }
  ...
  async upsertFile(file: Pick<Insertable<AssetFiles>, 'assetId' | 'path' | 'type'>): Promise<void> {
    const values = [{ ...file, assetId: asUuid(file.assetId) }];
    await upsertHelper(this.db, 'asset_files', values, ['assetId', 'type']).execute();
  }

  @GenerateSql({ params: [{ assetId: DummyValue.UUID, type: AssetFileType.PREVIEW, path: '/path/to/file' }] })
  async upsertFiles(file: Pick<Insertable<AssetFiles>, 'assetId' | 'path' | 'type'>[]): Promise<void> {
    const values = file.map((row) => ({ ...row, assetId: asUuid(row.assetId) }));
    await upsertHelper(this.db, 'asset_files', values, ['assetId', 'type']).execute();
  }
}

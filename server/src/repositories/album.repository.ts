...
export class AlbumRepository {
  ...
  private async addAssets(db: Kysely<DB>, albumId: string, assetIds: string[], createdBy: string): Promise<void> {
    ...
    await db
      ...
      .values(assetIds.map((assetId) => ({ albumsId: albumId, assetsId: assetId, createdBy })))
      .execute();
  }

  @Chunked({ chunkSize: 30_000 })
  async addAssetIdsToAlbums(values: { albumsId: string; assetsId: string; createdBy: string }[]): Promise<void> {
    if (values.length === 0) {
      return;
    }

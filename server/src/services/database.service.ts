...
export class DatabaseService extends BaseService {
  ...
  private async checkReindexing() {
    try {
      const indices = [VectorIndex.CLIP, VectorIndex.FACE];
      const [shouldReindexClip, shouldReindexFace] = await this.databaseRepository.shouldReindex(indices);

      const reindexing = [];
      if (shouldReindexClip) {
        reindexing.push(this.databaseRepository.reindex(VectorIndex.CLIP));
      }

      if (shouldReindexFace) {
        reindexing.push(this.databaseRepository.reindex(VectorIndex.FACE));
      }

      if (reindexing.length > 0) {
        await Promise.all(reindexing);
      }
    } catch (error) {
      this.logger.warn(

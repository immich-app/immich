import { ActivityService } from 'src/services/activity.service';
import { AiAutoAlbumService } from 'src/services/ai-auto-album.service';
import { AiDateEstimationService } from 'src/services/ai-date-estimation.service';
import { AiDescriptionService } from 'src/services/ai-description.service';
import { AiMemoryNarrativeService } from 'src/services/ai-memory-narrative.service';
import { AiSceneTagService } from 'src/services/ai-scene-tag.service';
import { AlbumService } from 'src/services/album.service';
import { ApiKeyService } from 'src/services/api-key.service';
import { ApiService } from 'src/services/api.service';
import { AssetMediaService } from 'src/services/asset-media.service';
import { BatchMetadataService } from 'src/services/batch-metadata.service';
import { AssetService } from 'src/services/asset.service';
import { AuditService } from 'src/services/audit.service';
import { AuthAdminService } from 'src/services/auth-admin.service';
import { AuthService } from 'src/services/auth.service';
import { CliService } from 'src/services/cli.service';
import { DatabaseBackupService } from 'src/services/database-backup.service';
import { DatabaseService } from 'src/services/database.service';
import { DownloadService } from 'src/services/download.service';
import { DuplicateService } from 'src/services/duplicate.service';
import { FrigateBridgeService } from 'src/services/frigate-bridge.service';
import { HaIntegrationService } from 'src/services/ha-integration.service';
import { JobService } from 'src/services/job.service';
import { LibraryService } from 'src/services/library.service';
import { LocationPrivacyService } from 'src/services/location-privacy.service';
import { MaintenanceService } from 'src/services/maintenance.service';
import { MapService } from 'src/services/map.service';
import { MediaService } from 'src/services/media.service';
import { MemoryService } from 'src/services/memory.service';
import { MetadataService } from 'src/services/metadata.service';
import { NlpSearchService } from 'src/services/nlp-search.service';
import { NotificationAdminService } from 'src/services/notification-admin.service';
import { NotificationService } from 'src/services/notification.service';
import { OcrService } from 'src/services/ocr.service';
import { PartnerService } from 'src/services/partner.service';
import { PersonService } from 'src/services/person.service';
import { PhotoQualityService } from 'src/services/photo-quality.service';
import { PluginService } from 'src/services/plugin.service';
import { QueueService } from 'src/services/queue.service';
import { SearchService } from 'src/services/search.service';
import { ServerService } from 'src/services/server.service';
import { SessionService } from 'src/services/session.service';
import { SharedLinkService } from 'src/services/shared-link.service';
import { SmartDuplicateResolutionService } from 'src/services/smart-duplicate-resolution.service';
import { SmartInfoService } from 'src/services/smart-info.service';
import { StackService } from 'src/services/stack.service';
import { StorageTemplateService } from 'src/services/storage-template.service';
import { StorageService } from 'src/services/storage.service';
import { StorageTieringService } from 'src/services/storage-tiering.service';
import { SyncService } from 'src/services/sync.service';
import { SystemConfigService } from 'src/services/system-config.service';
import { SystemMetadataService } from 'src/services/system-metadata.service';
import { TagService } from 'src/services/tag.service';
import { TelemetryService } from 'src/services/telemetry.service';
import { TimelineService } from 'src/services/timeline.service';
import { TrashService } from 'src/services/trash.service';
import { TripDetectionService } from 'src/services/trip-detection.service';
import { UserAdminService } from 'src/services/user-admin.service';
import { UserService } from 'src/services/user.service';
import { VersionService } from 'src/services/version.service';
import { VideoChaptersService } from 'src/services/video-chapters.service';
import { VideoHighlightsService } from 'src/services/video-highlights.service';
import { VideoTranscriptionService } from 'src/services/video-transcription.service';
import { ViewService } from 'src/services/view.service';
import { WebhookService } from 'src/services/webhook.service';
import { WorkflowService } from 'src/services/workflow.service';

export const services = [
  ApiKeyService,
  ActivityService,
  AiAutoAlbumService,
  AiDateEstimationService,
  AiDescriptionService,
  AiMemoryNarrativeService,
  AiSceneTagService,
  AlbumService,
  ApiService,
  AssetMediaService,
  AssetService,
  AuditService,
  AuthService,
  AuthAdminService,
  BatchMetadataService,
  CliService,
  DatabaseBackupService,
  DatabaseService,
  DownloadService,
  DuplicateService,
  FrigateBridgeService,
  HaIntegrationService,
  JobService,
  LibraryService,
  LocationPrivacyService,
  MaintenanceService,
  MapService,
  MediaService,
  MemoryService,
  MetadataService,
  NlpSearchService,
  NotificationService,
  NotificationAdminService,
  OcrService,
  PartnerService,
  PersonService,
  PhotoQualityService,
  PluginService,
  QueueService,
  SearchService,
  ServerService,
  SessionService,
  SharedLinkService,
  SmartDuplicateResolutionService,
  SmartInfoService,
  StackService,
  StorageService,
  StorageTemplateService,
  StorageTieringService,
  SyncService,
  SystemConfigService,
  SystemMetadataService,
  TagService,
  TelemetryService,
  TimelineService,
  TrashService,
  TripDetectionService,
  UserAdminService,
  UserService,
  VersionService,
  VideoChaptersService,
  VideoHighlightsService,
  VideoTranscriptionService,
  ViewService,
  WebhookService,
  WorkflowService,
];

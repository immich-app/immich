...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // Alter the phash column from bigint to bit(256)
  // Since user will re-add all photos, we don't need to preserve data
  await sql`
    ALTER TABLE asset_hash
    ALTER COLUMN "phash" TYPE bit(256) USING NULL
  `.execute(db);

  // Recreate the index on the new bit(256) column
  await sql`
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`
    ALTER TABLE asset_hash
    ALTER COLUMN "phash" TYPE bigint USING NULL
  `.execute(db);

...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`CREATE OR REPLACE TRIGGER "video_frame_extraction_updated_at"
  ...
  await sql`INSERT INTO "migration_overrides" ("name", "value") VALUES ('trigger_video_frame_extraction_updated_at', '{"type":"trigger","name":"video_frame_extraction_updated_at","sql":"CREATE OR REPLACE TRIGGER \\"video_frame_extraction_updated_at\\"\\n  BEFORE UPDATE ON \\"video_frame_extraction\\"\\n  FOR EACH ROW\\n  EXECUTE FUNCTION updated_at();"}'::jsonb);`.execute(db);
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`DROP TRIGGER "video_frame_extraction_updated_at" ON "video_frame_extraction";`.execute(db);
  await sql`ALTER TABLE "video_frame" DROP COLUMN "createdAt";`.execute(db);
  await sql`DELETE FROM "migration_overrides" WHERE "name" = 'trigger_video_frame_extraction_updated_at';`.execute(db);
}

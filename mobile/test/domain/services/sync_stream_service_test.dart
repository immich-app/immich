...
void main() {
  ...
  setUp(() async {
    ...
    ).thenAnswer((_) async => ServerVersionResponseDto(major: 1, minor: 132, patch: 0));
  ...
  group('SyncStreamService - Sync Migration', () {
    test('ensure that <2.5.0 migrations run', () async {
      ...
      when(
        () => mockServerApi.getServerVersion(),
      ).thenAnswer((_) async => ServerVersionResponseDto(major: 2, minor: 4, patch: 1));
    ...
    test('ensure that >=2.5.0 migrations run', () async {
      ...
      when(
        () => mockServerApi.getServerVersion(),
      ).thenAnswer((_) async => ServerVersionResponseDto(major: 2, minor: 5, patch: 0));
      await sut.sync();

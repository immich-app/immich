...
abstract final class StoreOld {
  ...
  static U? getOldValue<T, U>(StoreKey<T> key, [U? defaultValue]) {
    final StoreValue? value = _db.storeValues.getSync(key.id);
    final oldKey = StoreKeyOld.values.firstWhere((e) => e.id == key.id);
    return value?._extract(oldKey as StoreKeyOld<U>) ?? defaultValue;
  }

  static Future<void> updateVersion(int version) async {
    final StoreValue value =
        StoreValue(StoreKeyOld.version.id, intValue: version);
    await _db.writeTxn(() => _db.storeValues.put(value));
  }
}
...
class StoreValue {
  ...
  T? _extract<T>(StoreKeyOld<T> key) => switch (key.type) {
        ...
        _ when key.fromDb != null => key.fromDb!.call(StoreOld._db, intValue!),
        _ => throw TypeError(),
      };
}
...
class SSLClientCertStoreVal {
  ...
  static SSLClientCertStoreVal? load() {
    final b64Str = Store.tryGet(StoreKey.sslClientCertData);
    if (b64Str == null) {
      return null;
    }
    final Uint8List certData = base64Decode(b64Str);
    final passwd = Store.tryGet(StoreKey.sslClientPasswd);
...
enum StoreKeyOld<T> {
  ...
  const StoreKeyOld(this.id, {required this.type, this.fromDb, this.toDb});
  final int id;
  final Type type;
...
T? _getUser<T>(Isar db, int i) {
  return db.users.getSync(i)?.id as T?;
}

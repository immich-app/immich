...
final _stateProvider = Provider.family.autoDispose<_State?, ActionSource>((ref, source) {
  ...
  return (assetIds: ownedAssets.map((asset) => asset.id).toList(growable: false), origin: ownedAssets.singleOrNull);
});
...
class EditLocationAction extends AssetActionBuilder {
  ...
  Future<void> _edit(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      if (origin != null) {
        final exif = await ref.read(remoteAssetRepositoryProvider).getExif(origin.id);
      ...
      ref.read(assetsActionProvider(source).notifier).clearSelect();
    } catch (error, stack) {

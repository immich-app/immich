...
class PanoramaBadge extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    if (!asset.isImage || !isPanorama(ref, asset)) {
...
class _PanoramaViewerPageState extends State<PanoramaViewerPage> {
  ...
  // Painting waits for the crop, so the first frame is already centered on it
  ...
  void didChangeDependencies() {
    ...
    if (_imageStream == null) {
      ...
      _loadCrop().whenComplete(() {
        if (mounted) {
          setState(() => _cropLoaded = true);
        }
      }).ignore();
    }
  }
  ...
  Future<void> _loadCrop() async {
    ...
    try {
      ...
      // The XMP sits at the head of the file; the range keeps this to 128 KB (servers ignoring it send the full body)
      final response = await NetworkRepository.client.get(Uri.parse(url), headers: const {'range': 'bytes=0-131071'});
      xmp = String.fromCharCodes(response.bodyBytes);
    } catch (_) {
    ...
    setState(() {
      _crop = crop;
      // Start looking at the center of the captured area
      _longitude = 360 * (_crop.left + _crop.width / 2 - 0.5);
      _latitude = 90 - 180 * (_crop.top + _crop.height / 2);
    });
  }

...
class _AssetPageState extends ConsumerState<AssetPage> {
  ...
  void _showDetails() {
    ...
    _proxyScrollController.animateTo(_snapOffset, duration: kThemeAnimationDuration, curve: Curves.easeOutCubic);
  }
  ...
  bool _willClose(double scrollVelocity) {
    ...
    if (position.pixels >= _snapOffset) return false;

    final simulation = position.physics.createBallisticSimulation(position, scrollVelocity);
    return simulation != null && simulation.x(double.infinity) < SnapScrollPhysics.minSnapDistance;
  }
  ...
  void _onTapUp(BuildContext context, TapUpDetails details, PhotoViewControllerValue controllerValue) {
    if (!_showingDetails) _viewer.toggleControls();
  }
  ...
  Widget _buildPhotoView(
    ...
    if (displayAsset.isImage && !isPlayingMotionVideo) {
      ...
      return PhotoView(
        ...
        backgroundDecoration: BoxDecoration(color: _backgroundColor),
  ...
  Color get _backgroundColor => Colors.black.withValues(alpha: _backgroundOpacity);

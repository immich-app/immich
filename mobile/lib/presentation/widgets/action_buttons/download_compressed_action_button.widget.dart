...
import 'package:immich_mobile/widgets/asset_viewer/compressed_download_dialog.dart';

class DownloadCompressedActionButton extends ConsumerWidget {
  final ActionSource source;
  final bool iconOnly;
  final bool menuItem;

  const DownloadCompressedActionButton({
    super.key,
  ...
  void _onTap(BuildContext context, WidgetRef ref, BackgroundSyncManager backgroundSyncManager) async {
    ...
    final quality = await showDialog<int>(
      context: context,
      builder: (_) => const CompressedDownloadDialog(),
    );
    ...
    try {
      await ref.read(actionProvider.notifier).downloadCompressed(source, quality: quality);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return BaseActionButton(
      ...
      maxWidth: 140,
      label: 'download_compressed_jpeg'.t(context: context),
      iconOnly: iconOnly,
      menuItem: menuItem,

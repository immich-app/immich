...
class _DriftSlideshowPageState extends ConsumerState<DriftSlideshowPage> with SingleTickerProviderStateMixin {
  ...
  void _play() {
    ...
    if (asset.isImage) {
      _createTimer();
    } else if (ref.read(videoPlayerProvider(asset.playerKey)).status == VideoPlaybackStatus.paused) {
      unawaited(ref.read(videoPlayerProvider(asset.playerKey).notifier).play());
    } else {
      unawaited(_nextPage());
    }
  ...
  void _pause() {
    ...
    if (!asset.isImage) {
      unawaited(ref.read(videoPlayerProvider(asset.playerKey).notifier).pause());
    }
  ...
  Widget _getProgressBar(BuildContext context) {
    ...
    if (asset.isImage) {
      ...
      return LinearProgressIndicator(
        ...
        minHeight: 5,
        value:
            ref.read(videoPlayerProvider(asset.playerKey).select((s) => s.position)).inMilliseconds /
  ...
  Widget _getPhotoView(BuildContext context, int index) {
    ...
    if (asset.isImage) {
      ...
      final status = ref.read(videoPlayerProvider(asset.playerKey).select((s) => s.status));
      final position = ref.read(videoPlayerProvider(asset.playerKey)).position;

      if (status == VideoPlaybackStatus.completed && isCurrent && position.inMicroseconds > 0) {
        unawaited(_nextPage());
      } else if (status == VideoPlaybackStatus.playing) {
        unawaited(ref.read(videoPlayerProvider(asset.playerKey).notifier).setLoop(false));
      }

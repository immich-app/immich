...
class PaginatedSearch extends _$PaginatedSearch {
  Future<SearchResponseDto?> _search(SearchFilter filter, int page) async {
  ...
  Future<void> getNextPage(SearchFilter filter, int nextPage) async {
    ...
    final newState = await AsyncValue.guard(() async {
      final result = await _search(filter, nextPage);
      final assets = result?.assets.items.map((e) => Asset.remote(e)).toList();

      if (result != null && assets != null) {

...
(String msg, ToastType type) resolveAlbumAddToast(ActionResult result, String albumName, BuildContext context) {
  ...
  if (result.count > 0 && duplicate == 0) {
    return (context.t.add_to_album_bottom_sheet_added(album: albumName), ToastType.info);
  }

  if (result.count > 0 && duplicate > 0) {
    return (
      context.t.add_to_album_bottom_sheet_conflicts(added: result.count, album: albumName, failed: duplicate),
      ToastType.info,
    );
  }

  if (duplicate > 0) {
    return (context.t.add_to_album_bottom_sheet_already_exists(album: albumName), ToastType.info);
  }

  if ((result.failureReasons[BulkIdErrorReason.noPermission] ?? 0) > 0) {
    return (context.t.errors.add_to_album_no_permission, ToastType.error);
  }

  if ((result.failureReasons[BulkIdErrorReason.notFound] ?? 0) > 0) {
    return (context.t.errors.add_to_album_not_found, ToastType.error);
  }

  return (context.t.scaffold_body_error_occurred, ToastType.error);
}

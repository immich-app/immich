...
class MergedAssetDrift extends i1.ModularAccessor {
  ...
  i0.Selectable<MergedAssetResult> mergedAsset({
    ...
    return customSelect(
      ...
      (i0.QueryRow row) => MergedAssetResult(
        ...
        type: i3.$RemoteAssetEntityTable.$convertertype.fromSql(
          row.read<int>('type'),
        ),
  ...
  i0.Selectable<MergedBucketResult> mergedBucket({
    ...
    $arrayStartIndex += userIds.length;
    return customSelect(
      'SELECT COUNT(*) AS asset_count, bucket_date FROM (SELECT * FROM (SELECT rae.id AS remote_id, CASE WHEN ?1 IN (1, 2) THEN (SELECT lae.id FROM local_asset_entity AS lae WHERE lae.checksum = rae.checksum LIMIT 1) ELSE NULL END AS local_id, CASE WHEN ?2 = 0 THEN COALESCE(STRFTIME(\'%Y-%m-%d\', rae.local_date_time), STRFTIME(\'%Y-%m-%d\', rae.created_at, \'localtime\')) WHEN ?2 = 1 THEN COALESCE(STRFTIME(\'%Y-%m\', rae.local_date_time), STRFTIME(\'%Y-%m\', rae.created_at, \'localtime\')) END AS bucket_date FROM remote_asset_entity AS rae LEFT JOIN stack_entity AS se ON rae.stack_id = se.id WHERE rae.deleted_at IS NULL AND rae.visibility = 0 AND rae.owner_id IN ($expandeduserIds) AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT NULL AS remote_id, lae.id AS local_id, CASE WHEN ?2 = 0 THEN STRFTIME(\'%Y-%m-%d\', lae.created_at, \'localtime\') WHEN ?2 = 1 THEN STRFTIME(\'%Y-%m\', lae.created_at, \'localtime\') END AS bucket_date FROM local_asset_entity AS lae WHERE NOT EXISTS (SELECT 1 FROM remote_asset_entity AS rae WHERE rae.checksum = lae.checksum AND rae.owner_id IN ($expandeduserIds)) AND EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 0) AND NOT EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 2)) AS merged WHERE ?1 = 0 OR(?1 = 1 AND merged.remote_id IS NOT NULL AND merged.local_id IS NULL)OR(?1 = 2 AND merged.remote_id IS NOT NULL AND merged.local_id IS NOT NULL)OR(?1 = 3 AND merged.remote_id IS NULL)) GROUP BY bucket_date ORDER BY bucket_date DESC',
      variables: [
        i0.Variable<int>(filter),
  ...
  i3.$RemoteAssetEntityTable get remoteAssetEntity => i1.ReadDatabaseContainer(
    attachedDatabase,
  ).resultSet<i3.$RemoteAssetEntityTable>('remote_asset_entity');
  i4.$StackEntityTable get stackEntity => i1.ReadDatabaseContainer(
    attachedDatabase,
  ).resultSet<i4.$StackEntityTable>('stack_entity');
  i5.$LocalAssetEntityTable get localAssetEntity => i1.ReadDatabaseContainer(
    attachedDatabase,

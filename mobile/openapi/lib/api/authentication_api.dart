...
class AuthenticationApi {
  ...
  /// Performs an HTTP 'GET /auth/oauth-authorize' operation and returns the [Response].
  Future<Response> oauthAuthorizeWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/auth/oauth-authorize';
  ...
  Future<void> oauthAuthorize() async {
    final response = await oauthAuthorizeWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }

  /// Performs an HTTP 'GET /auth/oauth-callback' operation and returns the [Response].
  Future<Response> oauthAuthorizeCallbackWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/auth/oauth-callback';
  ...
  Future<LoginResponseDto?> oauthAuthorizeCallback() async {
    final response = await oauthAuthorizeCallbackWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.

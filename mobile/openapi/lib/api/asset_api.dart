...
class AssetApi {
  ...
  /// Performs an HTTP 'GET /asset/count' operation and returns the [Response].
  Future<Response> getAssetCountWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/asset/count';
  ...
  Future<AssetCountResponseDto?> getAssetCount() async {
    final response = await getAssetCountWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AssetCountResponseDto',) as AssetCountResponseDto;

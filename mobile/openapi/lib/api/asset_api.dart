...
class AssetApi {
  ...
  /// * [String] ifNoneMatch:
  ///   ETag of data already cached on the client
  Future<Response> getUserAssetsWithHttpInfo(String id, { String? ifNoneMatch, }) async {
    // ignore: prefer_const_declarations
    final path = r'/asset/partner/{id}'
      .replaceAll('{id}', id);
  ...
  Future<List<AssetResponseDto>?> getUserAssets(String id, { String? ifNoneMatch, }) async {
    final response = await getUserAssetsWithHttpInfo(id,  ifNoneMatch: ifNoneMatch, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.

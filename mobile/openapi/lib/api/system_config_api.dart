...
class SystemConfigApi {
  ...
  ///
  /// Render a storage template with sample data for preview purposes.
  ///
  /// Note: This method returns the HTTP [Response].
  ///
  /// Parameters:
  ///
  /// * [RenderStorageTemplateDto] renderStorageTemplateDto (required):
  Future<Response> renderStorageTemplateWithHttpInfo(RenderStorageTemplateDto renderStorageTemplateDto, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final apiPath = r'/system-config/storage-template-render';

    // ignore: prefer_final_locals
    Object? postBody = renderStorageTemplateDto;
  ...
  ///
  /// * [RenderStorageTemplateDto] renderStorageTemplateDto (required):
  Future<RenderStorageTemplateResponseDto?> renderStorageTemplate(RenderStorageTemplateDto renderStorageTemplateDto, { Future<void>? abortTrigger, }) async {
    final response = await renderStorageTemplateWithHttpInfo(renderStorageTemplateDto, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'RenderStorageTemplateResponseDto',) as RenderStorageTemplateResponseDto;

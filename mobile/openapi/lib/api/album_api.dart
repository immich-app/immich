...
class AlbumApi {
  ...
  /// Performs an HTTP 'PATCH /album/{id}/order' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] id (required):
  ///
  /// * [UpdateAlbumOrderDto] updateAlbumOrderDto (required):
  Future<Response> updateAlbumOrderWithHttpInfo(String id, UpdateAlbumOrderDto updateAlbumOrderDto,) async {
    // ignore: prefer_const_declarations
    final path = r'/album/{id}/order'
      .replaceAll('{id}', id);

    // ignore: prefer_final_locals
    Object? postBody = updateAlbumOrderDto;
  ...
  Future<UpdateAlbumDto?> updateAlbumOrder(String id, UpdateAlbumOrderDto updateAlbumOrderDto,) async {
    final response = await updateAlbumOrderWithHttpInfo(id, updateAlbumOrderDto,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'UpdateAlbumDto',) as UpdateAlbumDto;

...
class SharedLinksApi {
  ...
  /// Performs an HTTP 'GET /shared-links/unchecked' operation and returns the [Response].
  Future<Response> getAllSharedLinksUncheckedWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/shared-links/unchecked';
  ...
  Future<List<SharedLinkResponseDto>?> getAllSharedLinksUnchecked() async {
    final response = await getAllSharedLinksUncheckedWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.

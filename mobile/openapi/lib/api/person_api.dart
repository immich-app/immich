...
class PersonApi {
  ...
  /// Performs an HTTP 'GET /person/{id}/albums' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] id (required):
  Future<Response> getPersonAlbumsWithHttpInfo(String id,) async {
    // ignore: prefer_const_declarations
    final path = r'/person/{id}/albums'
      .replaceAll('{id}', id);
  ...
  Future<List<AlbumsForPersonResponseDto>?> getPersonAlbums(String id,) async {
    final response = await getPersonAlbumsWithHttpInfo(id,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<AlbumsForPersonResponseDto>') as List)
        .cast<AlbumsForPersonResponseDto>()
        .toList();

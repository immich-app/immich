...
class JobApi {
  ...
  /// Performs an HTTP 'POST /jobs/all' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [JobCommandDto] jobCommandDto (required):
  Future<Response> sendAllJobsCommandWithHttpInfo(JobCommandDto jobCommandDto,) async {
    // ignore: prefer_const_declarations
    final path = r'/jobs/all';
  ...
  ///
  /// * [JobCommandDto] jobCommandDto (required):
  Future<AllJobStatusResponseDto?> sendAllJobsCommand(JobCommandDto jobCommandDto,) async {
    final response = await sendAllJobsCommandWithHttpInfo(jobCommandDto,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.

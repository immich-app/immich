...
class MaintenanceAuthDto {
  ...
  static MaintenanceAuthDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'username'), 'Required key "MaintenanceAuthDto[username]" is missing from JSON.');
        assert(json[r'username'] != null, 'Required key "MaintenanceAuthDto[username]" has a null value in JSON.');
        return true;
      }());

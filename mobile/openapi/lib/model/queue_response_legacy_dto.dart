...
class QueueResponseLegacyDto {
  ...
  static QueueResponseLegacyDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'jobCounts'), 'Required key "QueueResponseLegacyDto[jobCounts]" is missing from JSON.');
        assert(json[r'jobCounts'] != null, 'Required key "QueueResponseLegacyDto[jobCounts]" has a null value in JSON.');
        assert(json.containsKey(r'queueStatus'), 'Required key "QueueResponseLegacyDto[queueStatus]" is missing from JSON.');
        assert(json[r'queueStatus'] != null, 'Required key "QueueResponseLegacyDto[queueStatus]" has a null value in JSON.');
        return true;
      }());

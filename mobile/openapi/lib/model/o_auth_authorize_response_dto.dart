...
class OAuthAuthorizeResponseDto {
  ...
  static OAuthAuthorizeResponseDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'url'), 'Required key "OAuthAuthorizeResponseDto[url]" is missing from JSON.');
        assert(json[r'url'] != null, 'Required key "OAuthAuthorizeResponseDto[url]" has a null value in JSON.');
        return true;
      }());

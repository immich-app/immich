...
class MergePersonDto {
  ...
  static MergePersonDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'ids'), 'Required key "MergePersonDto[ids]" is missing from JSON.');
        assert(json[r'ids'] != null, 'Required key "MergePersonDto[ids]" has a null value in JSON.');
        return true;
      }());

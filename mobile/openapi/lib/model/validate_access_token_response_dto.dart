...
class ValidateAccessTokenResponseDto {
  ...
  static ValidateAccessTokenResponseDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'authStatus'), 'Required key "ValidateAccessTokenResponseDto[authStatus]" is missing from JSON.');
        assert(json[r'authStatus'] != null, 'Required key "ValidateAccessTokenResponseDto[authStatus]" has a null value in JSON.');
        return true;
      }());

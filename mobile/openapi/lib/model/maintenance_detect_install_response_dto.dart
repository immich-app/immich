...
class MaintenanceDetectInstallResponseDto {
  ...
  static MaintenanceDetectInstallResponseDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'storage'), 'Required key "MaintenanceDetectInstallResponseDto[storage]" is missing from JSON.');
        assert(json[r'storage'] != null, 'Required key "MaintenanceDetectInstallResponseDto[storage]" has a null value in JSON.');
        return true;
      }());

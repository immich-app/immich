...
class QueueStatusLegacyDto {
  ...
  static QueueStatusLegacyDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'isActive'), 'Required key "QueueStatusLegacyDto[isActive]" is missing from JSON.');
        assert(json[r'isActive'] != null, 'Required key "QueueStatusLegacyDto[isActive]" has a null value in JSON.');
        assert(json.containsKey(r'isPaused'), 'Required key "QueueStatusLegacyDto[isPaused]" is missing from JSON.');
        assert(json[r'isPaused'] != null, 'Required key "QueueStatusLegacyDto[isPaused]" has a null value in JSON.');
        return true;
      }());

...
class SyncAlbumToAssetV1 {
  ...
  static SyncAlbumToAssetV1? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'albumId'), 'Required key "SyncAlbumToAssetV1[albumId]" is missing from JSON.');
        assert(json[r'albumId'] != null, 'Required key "SyncAlbumToAssetV1[albumId]" has a null value in JSON.');
        assert(json.containsKey(r'assetId'), 'Required key "SyncAlbumToAssetV1[assetId]" is missing from JSON.');
        assert(json[r'assetId'] != null, 'Required key "SyncAlbumToAssetV1[assetId]" has a null value in JSON.');
        return true;
      }());

...
class SystemConfigLoggingDto {
  ...
  static SystemConfigLoggingDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'enabled'), 'Required key "SystemConfigLoggingDto[enabled]" is missing from JSON.');
        assert(json[r'enabled'] != null, 'Required key "SystemConfigLoggingDto[enabled]" has a null value in JSON.');
        assert(json.containsKey(r'level'), 'Required key "SystemConfigLoggingDto[level]" is missing from JSON.');
        assert(json[r'level'] != null, 'Required key "SystemConfigLoggingDto[level]" has a null value in JSON.');
        return true;
      }());

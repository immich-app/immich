...
class SyncAssetOcrDeleteV1 {
  ...
  static SyncAssetOcrDeleteV1? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'assetId'), 'Required key "SyncAssetOcrDeleteV1[assetId]" is missing from JSON.');
        assert(json[r'assetId'] != null, 'Required key "SyncAssetOcrDeleteV1[assetId]" has a null value in JSON.');
        assert(json.containsKey(r'deletedAt'), 'Required key "SyncAssetOcrDeleteV1[deletedAt]" is missing from JSON.');
        assert(json[r'deletedAt'] != null, 'Required key "SyncAssetOcrDeleteV1[deletedAt]" has a null value in JSON.');
        assert(json.containsKey(r'id'), 'Required key "SyncAssetOcrDeleteV1[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "SyncAssetOcrDeleteV1[id]" has a null value in JSON.');
        return true;
      }());

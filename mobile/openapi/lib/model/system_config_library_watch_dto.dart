...
class SystemConfigLibraryWatchDto {
  ...
  static SystemConfigLibraryWatchDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'enabled'), 'Required key "SystemConfigLibraryWatchDto[enabled]" is missing from JSON.');
        assert(json[r'enabled'] != null, 'Required key "SystemConfigLibraryWatchDto[enabled]" has a null value in JSON.');
        return true;
      }());

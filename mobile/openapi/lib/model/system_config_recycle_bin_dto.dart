...
class SystemConfigRecycleBinDto {
  ...
  @override
  String toString() => 'SystemConfigRecycleBinDto[enabled=$enabled, days=$days]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SystemConfigRecycleBinDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SystemConfigRecycleBinDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<SystemConfigRecycleBinDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SystemConfigRecycleBinDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
